...
公開時は**4本ともREADME内で埋め込み再生**する構成です。日本語・英語とも、表やサムネイルではなく動画ごとの見出しを用意しています。
現在は公開先・添付URLが未確定のため、一時的に同梱MP4への相対リンクを置いています。埋め込み再生が完了した状態ではありません。
...
5. 両READMEの4か所を置き換えたら、`BEGIN VIDEO-UPLOAD-NOTICE` から `END VIDEO-UPLOAD-NOTICE` までの公開待ち案内を削除します。
...
添付URLの発行とGitHub上での再生確認は未実施です。ローカルのMarkdownプレビューだけではGitHubのプレイヤーを検証できません。
...
All four videos are intended to play inline in both READMEs, each under its own heading rather than in a thumbnail table.
The current relative MP4 links are temporary fallbacks until the destination repository and attachment URLs are available; inline playback is not complete.
...
To finalize, confirm the destination and visibility, then drop each MP4 into GitHub's README editor and wait for its attachment URL.
...
After all eight slots are updated, remove the `VIDEO-UPLOAD-NOTICE` block from both READMEs.
...
Release attachments are not required. Uploads and hosted playback verification are still pending; local Markdown previews do not verify GitHub playback.
