...
internal object PaperMonoProtocol {
    private const val MAGIC_0: Byte = 0x54
    private const val MAGIC_1: Byte = 0x43
    ...
    const val MAX_IMAGE_BYTES = 524288
    ...
    fun begin(transfer: PendingTransfer): ByteArray = buffer(23).apply {
        ...
        put(1.toByte()) // UTF-8 JSON envelope
    ...
    fun parseResponse(bytes: ByteArray, expected: PaperMonoCommand): PaperMonoResponse {
        if (bytes.size < RESPONSE_ENVELOPE_BYTES) {
            throw ProtocolException("SHORT_RESPONSE", "本体の応答が13バイト未満です。")
        }
        ...
        if (data.get() != VERSION) {
            throw ProtocolException("BAD_VERSION", "本体のプロトコルバージョンが一致しません。")
        }
        val command = data.get().toInt() and 0xff
        if (command != (expected.code or 0x80)) {
            throw ProtocolException("BAD_COMMAND", "本体の応答コマンドが一致しません。")
        }
    ...
    fun parseHello(response: PaperMonoResponse): HelloCapabilities {
        if (response.extra.size < 13) {
            throw ProtocolException("SHORT_HELLO", "本体のHELLO応答が短すぎます。")
        }
        if (response.extra[12].toInt() != 1) {
            throw ProtocolException("WRONG_SESSION", "本体の「設定 → スマホから更新」を開いてください。")
        }
        ...
        if (
            ...
            throw ProtocolException("UNSAFE_HELLO", "本体がv2の安全上限を超える能力値を返しました。")
        }
...
internal enum class PaperMonoStatus(val code: Int) {
    ...
    COMPLETED(0x15), NO_SPACE(0x16), MEDIA_LOST(0x17), WRONG_SESSION(0x18), UNKNOWN(0xff);
...
internal data class HelloCapabilities(
    ...
    val supportsJsonEnvelope: Boolean,
    val supportsTimeSync: Boolean,
    val supportsTypedUpdates: Boolean,
)
