...
{
  private static let methodChannelName = "io.github.corvelis.touch_card/methods"
  private static let eventChannelName = "io.github.corvelis.touch_card/events"
  ...
  private func localizedError(code: String, fallback: String) -> String {
    ...
    case "NO_SPACE", "noSpace":
      return text("保存容量が不足しています。名刺を削除するかSDを確認してください。", "Not enough space. Delete cards or check the SD card.")
    case "MEDIA_LOST", "mediaLost":
      return text("保存先のSDが取り外されました。", "The destination SD card was removed.")
    case "CONFLICT", "conflict":
      return text("名刺の更新世代が競合しています。保存し直してください。", "Card revisions conflict. Save your changes again.")
    case "WRONG_SESSION", "wrongSession":
      return text("本体の「設定 → スマホから更新」を開いてください。", "Open Settings → Phone update on the device.")
    case "INTERNAL_ERROR", "internalError":
      return text("本体への保存に失敗しました。ストレージを確認してください。", "Could not save to the device. Check its storage.")
    case "TIME_SYNC_UNSUPPORTED":
      return text("本体が時刻同期に対応していません。", "The device does not support clock sync.")
    case "FULLSCREEN_UNSUPPORTED":
      return text("本体が全画面画像に対応していません。", "The device does not support full-screen images.")
    case "INVALID_OFFSET", "TRANSFER_STALLED":
      return text("Paper Monoから不正な受信位置が返されました。", "The device returned an invalid transfer position.")
    case "TRANSFER_ID_MISMATCH":
      return text("本体の転送IDが一致しません。", "The device transfer ID does not match.")
    case "COMMIT_TIMEOUT":
      return text("本体の保存確認がタイムアウトしました。", "Timed out while waiting for the device to save.")
    case "CRC_MISMATCH":
      return text("本体で画像CRCが一致しませんでした。", "The data checksum did not match on the device.")
    case "INVALID_JPEG":
      return text("名刺または画像の形式が不正です。", "The device rejected the card or image format.")
    default:
  ...
  private func startTransfer(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    guard !bytes.isEmpty,
          ...
          transferId != 0, (1...4).contains(mode), width == 0, height == 0
    else {
      result(FlutterError(
        code: "INVALID_IMAGE",
        message: text("データサイズまたは転送IDが不正です。", "The image size or transfer ID is invalid."),
        details: nil
      ))
      return
    }
    ...
    guard beginReaderSession(
      ...
      message: text("本体にiPhoneを当ててください。", "Hold your iPhone near Paper Mono.")
    ) else {
      pendingTransfer = nil
      result(FlutterError(
  ...
  private func beginReaderSession(totalBytes: Int, message: String) -> Bool {
    ...
    newSession.alertMessage = lastSystemProgressPercent > 0
      ...
      : text("本体にiPhone上部を当ててください。", "Hold the top of your iPhone near the device.")
  ...
  func tagReaderSessionDidBecomeActive(_ session: NFCTagReaderSession) {
    ...
    session.alertMessage = lastSystemProgressPercent > 0
      ...
      : text("本体を探しています。iPhone上部を当ててください。", "Looking for Paper Mono. Hold the top of your iPhone near it.")
    emit(
      phase: "waitingForTag",
  ...
  func tagReaderSession(
    ...
    guard case let .miFare(tag) = first else {
      ...
      session.alertMessage = text("本体のMIFAREタグではありません。", "This is not a compatible MIFARE tag.")
      session.restartPolling()
      return
    }
    ...
    session.connect(to: first) { [weak self] error in
      ...
      session.alertMessage = text("本体に接続しました。送信を開始します。", "Connected to the device. Starting transfer.")
      self.emit(
        phase: "connected",
  ...
  private func sendTime(
    ...
    exchange(
      ...
      do {
        ...
        if response.status == .busy && attempts < 30 {
          DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) { [weak self] in
            guard let self, self.pendingTransfer != nil else { return }
            self.sendTime(tag: tag, transfer: transfer, attempts: attempts + 1)
          }
          return
        }
  ...
  private func sendImage(
    ...
    guard capabilities.supportsTypedUpdates else {
      failOrRecover(TransferError(
        code: "FULLSCREEN_UNSUPPORTED",
        message: "本体がTouch Cardの用途別更新に対応していません。"
      ))
      return
    }
    ...
    exchange(
      ...
      do {
        ...
        guard offset >= 0, offset <= transfer.bytes.count else {
          throw TransferError(code: "INVALID_OFFSET", message: "本体が不正な再開位置を返しました。")
        }
    ...
    exchange(
      ...
      do {
        ...
        guard next >= 0, next <= transfer.bytes.count else {
          throw TransferError(code: "INVALID_OFFSET", message: "本体が不正な受信位置を返しました。")
        }
        let nextStalls = next == offset ? stalls + 1 : 0
        guard nextStalls < 3 else {
          throw TransferError(code: "TRANSFER_STALLED", message: "本体の受信位置が進みません。")
        }
        self.emit(
          phase: "receiving",
          bytesSent: next,
      ...
      self.exchange(
        ...
        do {
          ...
          if response.status == .completed {
            ...
            throw TransferError(code: "COMMIT_TIMEOUT", message: "本体の保存確認がタイムアウトしました。")
          } else {
            self.pollCommit(
              tag: tag,
              transfer: transfer,
  ...
  private func handleCommitStatus(
    ...
    case .ok, .accepted, .verifying:
      session?.alertMessage = text("送信完了: 100%　CRCとデータを検証しています。", "Transfer complete: 100%. Verifying CRC and image.")
      emit(
        phase: "verifying",
    ...
    case .stored:
      session?.alertMessage = text("データを保存しました。画面更新を待っています。", "Data stored. Waiting for the display update.")
      emit(
        phase: "stored",
        bytesSent: transfer.bytes.count,
        totalBytes: transfer.bytes.count,
    ...
    case .displaying:
      session?.alertMessage = text("本体の画面を更新しています。", "Updating the device display.")
      emit(
        phase: "displaying",
        bytesSent: transfer.bytes.count,
        totalBytes: transfer.bytes.count,
    ...
    case .completed:
      break
    case .crcMismatch:
      throw TransferError(code: "CRC_MISMATCH", message: "本体で画像CRCが一致しませんでした。")
    case .invalidJpeg:
      throw TransferError(code: "INVALID_JPEG", message: "名刺または画像の形式が不正です。")
    default:
      throw TransferError(
        code: String(describing: response.status),
        message: "本体がCOMMITを拒否しました: \(response.status)"
      )
    }
  }
  ...
  private func updateSystemProgress(bytesSent: Int, totalBytes: Int) {
    ...
    session?.alertMessage = String(
      format: text("データを送信中: %d%%（%.1f / %.1f KB）", "Sending data: %d%% (%.1f / %.1f KB)"),
      percent,
  ...
  private func exchange(
    ...
    tag.sendMiFareCommand(
      ...
      do {
        ...
        // rejection. Every v2 command is idempotent, so retry the exact same
  ...
  private func requireTransferId(
    ...
    guard response.transferId == transfer.transferId else {
      ...
      throw TransferError(code: "TRANSFER_ID_MISMATCH", message: "本体の転送IDが一致しません。")
    }
  }
  ...
  private func requireStatus(
    ...
    guard accepted.contains(response.status) else {
      throw TransferError(
        code: String(describing: response.status),
        message: "本体がコマンドを拒否しました: \(response.status)"
      )
    }
  }
  ...
  private func finish(transfer: PendingTransfer, completed: Bool) {
    session?.alertMessage = completed
      ? text("本体へのデータ送信が完了しました。", "Data transfer to the device completed.")
      : text("データを本体へ保存しました。", "Data stored on the device.")
    pendingTransfer = nil
    activeTag = nil
  ...
  private func finishClockSync() {
    session?.alertMessage = text("本体の時刻を同期しました。", "Device clock synchronized.")
    pendingTransfer = nil
    activeTag = nil
  ...
  private func completeTerminalAction(_ action: SessionTerminalAction) {
    ...
    case .clockSynced:
      emit(phase: "clockSynced", message: "本体の時刻を同期しました。")
    }
  }
  ...
  private func recoverFromCommunicationError(_ message: String) {
    ...
    emit(
      ...
      totalBytes: pendingTransfer?.bytes.count ?? 0,
      message: "\(message) もう一度本体へ当てると途中から再開します。",
      errorCode: "TAG_LOST"
    )
    session?.alertMessage = text("接続が切れました。もう一度本体へ当ててください。", "Connection lost. Hold your iPhone near the device again.")
    emit(
      phase: "waitingForTag",
      totalBytes: pendingTransfer?.bytes.count ?? 0,
      message: "NFCを再準備しています。そのまま本体へ当ててください。"
    )
  ...
  private func emit(
    ...
    transferId: UInt32? = nil
  ) {
    var event: [String: Any] = [
      "phase": phase,
      "transferId": transferId ?? pendingTransfer?.transferId ?? 0,
      "bytesSent": bytesSent,
      "totalBytes": totalBytes,
...
private struct HelloCapabilities {
  ...
  let supportsJsonEnvelope: Bool
  let supportsTimeSync: Bool
  let supportsTypedUpdates: Bool
}
...
private enum PaperMonoProtocol {
  ...
  static func begin(_ transfer: PendingTransfer) -> Data {
    ...
    data.append(1) // UTF-8 JSON envelope
  ...
  static func parseResponse(
    ...
    guard bytes.count >= responseEnvelopeBytes else {
      throw TransferError(code: "SHORT_RESPONSE", message: "本体の応答が13バイト未満です。")
    }
    ...
    guard bytes[2] == version else {
      throw TransferError(code: "BAD_VERSION", message: "本体のプロトコルバージョンが一致しません。")
    }
    guard bytes[3] == (expected.rawValue | 0x80) else {
      throw TransferError(code: "BAD_COMMAND", message: "本体の応答コマンドが一致しません。")
    }
    guard let status = PaperMonoStatus(rawValue: bytes[4]) else {
      throw TransferError(code: "UNKNOWN_STATUS", message: "本体が未知のステータスを返しました。")
    }
  ...
  static func parseHello(_ response: PaperMonoResponse) throws -> HelloCapabilities {
    guard response.extra.count >= 13 else {
      throw TransferError(code: "SHORT_HELLO", message: "本体のHELLO応答が短すぎます。")
    }
    guard response.extra[12] == 1 else {
      throw TransferError(code: "WRONG_SESSION", message: "本体の「設定 → スマホから更新」を開いてください。")
    }
    ...
    let capabilities = HelloCapabilities(
      ...
      supportsJsonEnvelope: capabilityBits & 0x01 != 0,
      supportsTimeSync: capabilityBits & 0x02 != 0,
      supportsTypedUpdates: capabilityBits & 0x04 != 0
    )
    guard capabilities.maxAcceptedRfFrameBytes <= 255,
      ...
      throw TransferError(code: "UNSAFE_HELLO", message: "本体がv2の安全上限を超える能力値を返しました。")
    }
    return capabilities
  }
