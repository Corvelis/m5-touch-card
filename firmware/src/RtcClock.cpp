...
namespace {
constexpr char kPreferencesNamespace[] = "touch_time";
...
bool validRtcFields(const m5::rtc_date_t &date, const m5::rtc_time_t &time) {
  return date.year >= 2023 && date.year <= 2099 && date.month >= 1 &&
...
bool RtcClock::sync(uint64_t unixSeconds, int16_t utcOffsetMinutes) {
  ...
  // M5Unified's setter is void: read back before acknowledging durable clock
  // synchronization. Allow the RTC to tick while I2C transactions complete.
  m5::rtc_date_t checkDate;
  m5::rtc_time_t checkTime;
  if (!M5.Rtc.getDateTime(&checkDate, &checkTime) ||
      !validRtcFields(checkDate, checkTime))
    return false;
  tm checked = {};
  checked.tm_year = checkDate.year - 1900;
  checked.tm_mon = checkDate.month - 1;
  checked.tm_mday = checkDate.date;
  checked.tm_hour = checkTime.hours;
  checked.tm_min = checkTime.minutes;
  checked.tm_sec = checkTime.seconds;
  setenv("TZ", "UTC0", 1);
  tzset();
  const time_t persisted = mktime(&checked);
  if (persisted < raw || persisted > raw + 2)
    return false;
...
bool RtcClock::valid() const { return valid_; }

int16_t RtcClock::utcOffsetMinutes() const { return utcOffsetMinutes_; }
...
bool RtcClock::nfcTimeSync(uint64_t unixSeconds, int16_t utcOffsetMinutes,
                           void *context) {
